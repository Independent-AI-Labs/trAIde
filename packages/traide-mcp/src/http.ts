  ...
  const server = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/health') {
        ...
        return void res.end(JSON.stringify({ status: 'ok', uptime: process.uptime(), version: '0.1.0', provider: 'binance', requestId: reqId }))
      }
      ...
      // GET /klines/batch?symbols=AAA,BBB&interval=1m&limit=60
      if (req.method === 'GET' && url.pathname === '/klines/batch') {
        metrics.inc('http_requests_total', { route: 'klines_batch', method: 'GET' })
        const symbolsParam = url.searchParams.get('symbols') ?? ''
        const interval = url.searchParams.get('interval') ?? ''
        const limit = url.searchParams.get('limit')
        const symbols = symbolsParam.split(',').map((s) => s.trim().toUpperCase()).filter(Boolean)
        if (!symbols.length || !interval) {
          res.writeHead(400, { 'content-type': 'application/json' })
          metrics.inc('http_requests_total', { route: 'klines_batch', method: 'GET', status: '400' })
        ...
        const run = async (): Promise<void> => {
          ...
          if (!sym) return
          try {
            const candles = await withTiming('http_latency_seconds', () => provider.getKlines({ symbol: sym, interval, start: undefined, end: undefined, limit: limit ? +limit : undefined }))
            out[sym] = candles
          } catch {
            out[sym] = []
          }
        ...
        await Promise.all(Array.from({ length: Math.min(max, symbols.length) }, () => run()))
        res.writeHead(200, { 'content-type': 'application/json' })
        metrics.inc('http_requests_total', { route: 'klines_batch', method: 'GET', status: '200' })
        return void res.end(JSON.stringify({ interval, result: out, requestId: reqId }))
      }
      ...
      // GET /stream/klines (single symbol)
      ...
      // GET /stream/klines/batch (multiple symbols multiplexed)
      if (req.method === 'GET' && url.pathname === '/stream/klines/batch') {
        metrics.inc('http_requests_total', { route: 'stream_klines_batch', method: 'GET' })
        const symbolsParam = url.searchParams.get('symbols') ?? ''
        const interval = url.searchParams.get('interval') ?? '1m'
